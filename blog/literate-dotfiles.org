...
#+HUGO_BUNDLE: literate-dotfiles
#+HUGO_SLUG: literate-dotfiles
#+HUGO_TAGS: software devops
...
source format for documentation and static {{{abbr(HTML,HyperText Markup Language)}}} generation on popular
...
- <2021-07-11 Sun> Update links to Emacs configuration. Since the
  ...
  configuration into my .files repository - {{{abbr(IE,that is)}}} they're all one
  repository. This change updates the hyperlinks to make sure the reader
  lands in the correct location.

- <2026-07-04 Sat> I no longer use the literate approach described here.
  Since this writing I've refactored both my =.files= and Emacs
  configuration into hand-written, modular Emacs Lisp files rather than
  tangling them from a literate org document. The "Emacs configuration"
  link above is pinned to a commit from the original literate version, so
  it still reflects the approach this article describes; the =.files=
  repository itself now reflects the newer modular layout.

* Glossary

- literate programming :: writing code and a human-readable explanation of it together in a single document.
- *nix :: Unix and Unix-like operating systems (for example Linux and macOS).
- org-mode :: an Emacs document format for notes and writing that can also hold and run embedded code.
- Babel :: the Emacs org-mode feature that runs embedded code blocks and extracts them into real files.
- tangling :: extracting the code blocks from a literate document into standalone source files.
- woven :: the readable, formatted document produced from a literate source, as opposed to the extracted code.
- git forge :: a website that hosts version-controlled code repositories, such as GitHub or GitLab.
