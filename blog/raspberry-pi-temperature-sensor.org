...
#+HUGO_BUNDLE: raspberry-pi-temperature-sensor
#+HUGO_SLUG: raspberry-pi-temperature-sensor
#+HUGO_TAGS: hardware "raspberry pi"
...
#+begin_plainsummary
In short: I wired a small temperature sensor to a Raspberry Pi, wrote a short Python program to read the temperature once a minute, saved the readings in a time-series database (InfluxDB), and drew a live graph of them with Grafana.
#+end_plainsummary

A few weeks ago, I purchased a Raspberry Pi. After reading [[http://www.amazon.com/Raspberry-User-Guide-Eben-Upton/dp/1118921666/ref=sr_1_1?ie=UTF8&qid=1417487615&sr=8-1&keywords=raspberry+pi+beginners+guide][Eben Upton's Book - Raspberry Pi User Guide]], particularly the two chapters on which he focuses attention on the {{{abbr(GPIO,General-Purpose Input/Output)}}} pins, I had an idea for my first project. The post covers the first iteration of a home temperature monitoring project I put together using a Raspberry Pi, a [[https://www.adafruit.com/products/1782][MCP9808 Temperature sensor]], an old Mac Mini (early 2008), [[http://influxdb.com][the InfluxDB time series database]], [[http://grafana.org][Grafana time series visualization applcation]], a bit of [[https://www.python.org][the Python programming language]], and [[http://smarden.org/runit/][the runit init system]].

#+CAPTION: Raspberry Pi with MCP9808 temperature sensor
#+NAME: Raspberry Pi Temperature Sensor
#+ATTR_HTML: :alt A Raspberry Pi wired by a ribbon cable to a small sensor breakout board on a desk, beside a keyboard and mouse.
[[../assets/blog/raspberry-pi-temperature-sensor/raspberrypi_mcp9808.png]]


#+CAPTION: Grafana Dashboard with temperature data
#+NAME: Grafana Dashboard Preview
#+ATTR_HTML: :alt A Grafana dashboard titled MCP9808 Temperature Readings, graphing room temperature over about a day between roughly 71 and 73.5 degrees Fahrenheit.
[[../assets/blog/raspberry-pi-temperature-sensor/grafana_dashboard_preview.png]]
...
For this project I chose to use the MCP9808 Breakout Board from [[http://www.adafruit.com][Adafruit - an excellent online resource for electronics components, circuits, and ideas]]. I chose this unit for a few reasons:
  ...
  1. It's controlled over {{{abbr(I²C,Inter-Integrated Circuit)}}} -- Raspberry Pi's GPIO supports the require I²C bus over pins 3 (Serial Data Line, SDA) and 5 (Serial Clock Line, SCL)
...
#+NAME: MCP9808 Sensor
#+ATTR_HTML: :alt The MCP9808 sensor, a small blue Adafruit breakout board, held next to a house key for scale.
[[../assets/blog/raspberry-pi-temperature-sensor/mcp9808.png]]
...
#+ATTR_HTML: :alt Wiring from the Raspberry Pi B+ 40-pin GPIO header to the MCP9808 breakout board: pin 1 (3.3V) to VDD, pin 3 (SDA) to SDA, pin 5 (SCL) to SCL, and pin 6 (GND) to GND. The table below lists the same connections.
[[../assets/blog/raspberry-pi-temperature-sensor/mcp9808_wiring_diagram.png]]
...
1. Create the new database; I named mine =mcp9808_test=
  #+CAPTION: Creating the mcp9808_test database in the InfluxDB web console
  #+NAME: Influx Create Database
  #+ATTR_HTML: :alt The InfluxDB web console Databases page, listing the grafana and mcp9808_test databases above a Create a Database form.
  [[../assets/blog/raspberry-pi-temperature-sensor/influxdb_create_database.png]]
2. Create the database admin user; I named mine =mcp9808=
   - Click on the database name in the Databases list view
   - Create a New Database User
     #+CAPTION: Creating the mcp9808 database user in the InfluxDB web console
     #+NAME: Influx Create Database User
     #+ATTR_HTML: :alt The InfluxDB console for the mcp9808_test database, showing an admin user named mcp9808 above a Create a New Database User form.
     [[../assets/blog/raspberry-pi-temperature-sensor/influxdb_database_users.png]]
...
** InfluxDB Graphing
  ...
  #+NAME: Influx Query
  #+ATTR_HTML: :alt The InfluxDB query console running a SELECT of the minimum degrees_f from Temperature Readings over the last day grouped by the hour, with the resulting line graph and a table of timestamped minimum temperatures below.
  [[../assets/blog/raspberry-pi-temperature-sensor/influxdb_query.png]]
...
** Grafana
  ...
  2. Define the metric query and graph attributes
    ...
    #+ATTR_HTML: :alt The Grafana graph editor Metrics tab, defining a query that selects max(degrees_f) from the Temperature Readings series grouped every minute, above the resulting temperature graph.
    [[../assets/blog/raspberry-pi-temperature-sensor/grafana_metric_query.png]]
  3. Return to the dashboard and select a time period against which the query should be run and an auto refresh interval
    ...
    #+ATTR_HTML: :alt The Grafana dashboard with the auto-refresh menu open and Every 1m selected, showing the temperature graph on a rolling one-day window.
    [[../assets/blog/raspberry-pi-temperature-sensor/grafana_dashboard.png]]
...
- breakout board :: a small circuit board that exposes a chip's pins so they are easy to wire up.
- breadboard :: a reusable board for building circuits by plugging in parts, with no soldering.
- "Cobbler" GPIO breakout :: an adapter that routes the Raspberry Pi's pins out to a breadboard for wiring.
- time-series database :: a database built to store readings indexed by the time they occurred.
- init system :: the first program the operating system starts; it launches and supervises long-running services.
- kernel modules :: loadable pieces of code that add features, such as hardware support, to the operating system core.
